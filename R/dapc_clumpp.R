...
run_dapc_clumpp <- function(data, CLUMPP_exe, modelNumber, k, model, perc.var, scale, center, clust.method, apriori){
  if(clust.method == "kmeans"){
	  ...
	  if(apriori == FALSE){
		  ...
		  dapc.apriori <- adegenet::dapc(data, model, var.conrib=TRUE, var.loadings=TRUE, perc.pca=perc.var, n.da=10000, center=center, scale=scale)
	...
	# DEVELOPMENTAL CODE for random forest clustering
